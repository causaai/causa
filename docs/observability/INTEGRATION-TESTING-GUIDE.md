# Datadog Integration Testing Guide
...
### Option A: Use Existing Image (Recommended for Testing)
```bash
...
# Create the required database secret
cat <<EOF | oc apply -f -
apiVersion: v1
kind: Secret
metadata:
...
stringData:
  username: causa_user
  password: causa_password_change_me
EOF
